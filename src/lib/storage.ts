...
export class Store {
  ...
  async save(type: 'prompt' | 'program', name: string, data: any): Promise<string> {
    ...
    debug('persistence', `--------------- SAVE OPERATION START ---------------`);
    debug('persistence', `Saving ${type} with name: ${name}`);
    debug('persistence', `Current base directory: ${this.baseDir}`);
    debug('persistence', `Process ID: ${process.pid}`);
    debug('persistence', `Current working directory: ${process.cwd()}`);

    // Create directories if needed
    const typeDir = path.join(this.baseDir, type + 's');
    const itemDir = path.join(typeDir, name);

    debug('persistence', `Type directory path: ${typeDir}`);
    debug('persistence', `Item directory path: ${itemDir}`);

    // Check if directories already exist before trying to create them
    const baseDirExists = await fsPromises.access(this.baseDir).then(() => true).catch(() => false);
    ...
    debug('persistence', `Directory existence check - Base: ${baseDirExists}, Type: ${typeDirExists}, Item: ${itemDirExists}`);

    // Use the enhanced ensureDir method to create all necessary directories
    try {
      // First ensure the base directory exists
      debug('persistence', `Creating base directory: ${this.baseDir}`);
      await this.ensureDir(this.baseDir);

      // Then ensure the type directory exists
      debug('persistence', `Creating type directory: ${typeDir}`);
      await this.ensureDir(typeDir);

      // Finally ensure the item directory exists
      debug('persistence', `Creating item directory: ${itemDir}`);
      ...
      debug('persistence', `Stack trace: ${err.stack}`);
    ...
    try {
      ...
      debug('persistence', `Starting version file save phase`);
      for (let saveAttempt = 0; saveAttempt < MAX_SAVE_RETRIES; saveAttempt++) {
        try {
          debug('persistence', `Saving version file (attempt ${saveAttempt + 1}/${MAX_SAVE_RETRIES}): ${versionPath}`);
          // Save version file
          ...
          debug('persistence', `Waiting for filesystem sync after write...`);
          await new Promise(resolve => setTimeout(resolve, 50)); // Increased delay

          // Verify it was saved correctly
          debug('persistence', `Verifying version file was created: ${versionPath}`);
          ...
          if (!versionExists) {
            ...
            if (saveAttempt < MAX_SAVE_RETRIES - 1) {
              debug('persistence', `Retrying after delay...`);
          ...
          debug('persistence', `Version file exists: ${versionExists}`);
      ...
      debug('persistence', `Starting latest pointer update phase`);
      debug('persistence', `Updating latest pointer for ${type} "${name}" to ${versionId}`);
      debug('persistence', `Latest pointer path: ${latestPath}`);
      ...
      for (let latestAttempt = 0; latestAttempt < MAX_LATEST_RETRIES; latestAttempt++) {
        try {
          debug('persistence', `Writing latest pointer file (attempt ${latestAttempt + 1}/${MAX_LATEST_RETRIES})`);
          ...
          debug('persistence', `Waiting for filesystem sync after latest pointer write...`);
          await new Promise(resolve => setTimeout(resolve, 50)); // Increased delay

          // Verify it was saved correctly
          debug('persistence', `Verifying latest pointer file was created: ${latestPath}`);
          ...
          debug('persistence', `Latest pointer file exists: ${latestExists}`);
