...
import { 
  withPrompt, withModel, withOptions, withPersistence, withExamples, withReturnsType 
...
describe('Pure builder methods', () => {
  it('withPrompt should set prompt immutably', () => {
    ...
    const state2 = withPrompt(state1, 'my prompt');
  ...
  it('withModel should set model immutably', () => {
    const state1 = createState();
    const state2 = withModel(state1, 'gpt-4');
    expect(state2).not.toBe(state1);
  ...
  it('withOptions should merge and set options immutably', () => {
    ...
    const state2 = withOptions(state1, { b: 2 });
    expect(state2).not.toBe(state1);
  ...
  it('withPersistence should merge and set persistence immutably', () => {
    ...
    const state2 = withPersistence(state1, { version: 2, extra: true });
    expect(state2).not.toBe(state1);
  ...
  it('withExamples should set examples immutably', () => {
    const state1 = createState();
    const examples = [{ input: 1, output: 2 }];
    const state2 = withExamples(state1, examples);
    expect(state2).not.toBe(state1);
    expect(state2.examples).toEqual(examples);
    expect(state1.examples).toBeUndefined();
  });

  it('withReturnsType should set returnsType immutably', () => {
    const state1 = createState();
    const state2 = withReturnsType(state1, { foo: 'bar' });
    expect(state2).not.toBe(state1);
  ...
  it('should allow chaining builder methods', () => {
    const state1 = createState();
    const state2 = withPrompt(state1, 'prompt');
    const state3 = withModel(state2, 'model');
    const state4 = withOptions(state3, { x: 1 });
